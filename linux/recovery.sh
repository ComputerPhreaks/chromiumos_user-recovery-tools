...
# it would create the second argument as a directory instead and put the file(s) in it
unzip() {
  local recoveryzip
  recoveryzip="$1"
  local destpath
...
# TODO: If your search doesn't return results it only offers 0 - <quit> as an option instead of having you search again
choose_image() {
  local show
  local count
  local line
